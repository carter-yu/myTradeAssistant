...
Select 
 op.StockTicker, op.ListedExchCode, op.Source, 
 ( (np.ClosePrice/op.ClosePrice) - 1) priceChg,
 op.DataAsOfDate, op.OpenPrice, op.ClosePrice, # op.Volume, op.52WkHigh, op.52WkLow, 
 np.DataAsOfDate, np.OpenPrice, np.ClosePrice, np.Volume, np.52WkHigh, np.52WkLow
From 
 ( Select * From mytradeassistantdb.tblStockDailyPrice Where DataAsOfDate = '2018-02-18' ) op,
 ( Select * From mytradeassistantdb.tblStockDailyPrice Where DataAsOfDate = '2018-04-05' ) np
Where 1=1
And op.StockTicker = np.StockTicker
And op.ListedExchCode = np.ListedExchCode
And op.Source = np.Source
Order by priceChg desc
;
