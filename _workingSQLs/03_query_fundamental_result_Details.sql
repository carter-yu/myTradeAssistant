...
SELECT DISTINCT
   ...
   ( sp.ClosePrice * ((18.78 * sv.Beta) / sv.Pe) ) as Fair_Price_PE,
   ( (( sp.ClosePrice * ((18.78 * sv.Beta) / sv.Pe) ) / sp.ClosePrice) - 1 ) as Fair_Price_PE_vs_Close_Price,
   #price * ((benchmark_peg * compBeta) / PEG_Reuters)
   (sp.ClosePrice * ( ((18.78 / 11.6) * sv.Beta / ( sv.Pe / (rsr.EpsRegressGrowth * 100) )) )) as Fair_Price_PEG_Reuters,
   (( (sp.ClosePrice * ( ((18.78 / 11.6) * sv.Beta / ( sv.Pe / (rsr.EpsRegressGrowth * 100) )) )) / sp.ClosePrice ) - 1) as Fair_Price_PEG_Reuters_vs_Close_Price,
   (sp.ClosePrice * ( ((18.78 / 11.6) * sv.Beta / ( sv.Pe / (nsr.EpsRegressGrowth) )) )) as Fair_Price_PEG_Nasdaq,
   (( (sp.ClosePrice * ( ((18.78 / 11.6) * sv.Beta / ( sv.Pe / (nsr.EpsRegressGrowth) )) )) / sp.ClosePrice ) - 1)  as Fair_Price_PEG_Nasdaq_vs_Close_Price,
   sp.ClosePrice, sp.52WkHigh, sp.52WkLow, ((sp.Volume / sp.AvgVolume) - 1) as VolumeChange, sp.Volume, sp.AvgVolume, 
   -- DJ PE: 29.961
   -- DJ PE: 18.78 as of 09 Oct
   #Cash Flow
...
## Applied Strategic filtering 
   ...
   AND (
        ...
        ( (( sp.ClosePrice * ((18.78 * sv.Beta) / sv.Pe) ) / sp.ClosePrice) - 1 ) 
        OR
        # as Fair_Price_PEG_Reuters_vs_Close_Price,
        (( (sp.ClosePrice * ( ((18.78 / 11.6) * sv.Beta / ( sv.Pe / (rsr.EpsRegressGrowth * 100) )) )) / sp.ClosePrice ) - 1) 
        OR
        # as Fair_Price_PEG_Nasdaq_vs_Close_Price,
        (( (sp.ClosePrice * ( ((18.78 / 11.6) * sv.Beta / ( sv.Pe / (nsr.EpsRegressGrowth) )) )) / sp.ClosePrice ) - 1) 
       )
